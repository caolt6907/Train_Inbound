...
//	 private  String sb = "crm/3d1ba8247e25/";
//	    private static String prompt = sb + "Talk.wav";
//	    private static String bye = "dialer/8000/" + "Bye.wav";
//	 private static Jedis rs = RedisUtils.getRs();//获取redis对象
			...
			//		System.out.println("$$$$$$$"+userDate+"&&&&&&&");
				...
				//	System.out.println("StrategyId="+"weviking_callcenter_StrategySubject_"+StrategyId);
					...
					/*	Content=Util.replaceTTs(Content, tts);//替换content变量文本					
						File fileSrc = new File(sb+LyCode+".wav");//这里文件路径有问题
						...
						*/
						try{
						 ...
						 Util.SendKafka(callid,projectId,mobile, "",2,JsonUtils.parseJSON2Map(Speech).get("speech")+"","", producer,Intention);//向kafka发送拨打过程
					...
					if(Type.equals("内容")){
						 Speech=client.exec.playAndDetectSpeech(sb+LyCode+".wav", AsrName,grammer,params);
						 Util.SendKafka(callid,projectId,mobile, "",2,JsonUtils.parseJSON2Map(Speech).get("speech")+"",LyCode+".wav", producer,Intention);//向kafka发送拨打过程
						 ...
						 */
						 ...
						 lastLy=LyCode;
						 Util.SendKafka(callid,projectId,mobile, "",2,JsonUtils.parseJSON2Map(Speech).get("speech")+"",LyCode+".wav", producer,Intention);//向kafka发送拨打过程
						if((JsonUtils.parseJSON2Map(Speech).get("speech")+"").equals("")){ 
						 if(!NextSubjectId.equals("")){
							 Speech= client.exec.playAndDetectSpeech(sb+NextSubjectId+".wav", AsrName,grammer,params);
							 Util.SendKafka(callid,projectId,mobile, "",2,JsonUtils.parseJSON2Map(Speech).get("speech")+"",LyCode+".wav", producer,Intention);//向kafka发送拨打过程
							 lastLy=NextSubjectId;
							 Type="";
						 }else{
							 Speech=client.exec.playAndDetectSpeech(sb+lastLy+".wav", AsrName,grammer,params);
			...
			// if(rs.get(map.get("ProjectId")+"_status")==null && Util.compare_date(map.get("StartTime").toString(), map.get("EndTime").toString()) ) {	//该项目没有在拨打同时时间到了			
			 ...
			 if(!(queueMap.get(map.get("ProjectId")+"_queue")==null)){ 
			 		if(Util.compare_date(map.get("StartTime").toString(), map.get("EndTime").toString())){//该项目已开始拨打，同时在工作时间内
			 			...
			 			/*	File fileSrc = new File(filePath+map.get("ProjectId")+"_thread"+i+".mp3"); //删除该项目下线程产生的MP3文件
			 				if (fileSrc.exists()){
			 					fileSrc.delete();
			 				}
			 				*/
